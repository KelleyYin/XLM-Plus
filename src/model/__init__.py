...
class wordEmbedding(nn.Module):
    def __init__(self, params, dico, embed_dim):
        super().__init__()
        self.params = params
        self.dico = dico
    ...
    if params.encoder_only:
        ...
        if params.share_word_embeddings or params.share_all_embeddings:
            ...
            if len(params.mt_steps) <= 1:
                ...
                print('sorry')
                exit()
                # encoder = TransformerModel(params, dico, emb, is_encoder=True, with_output=False, word_emb=)  # TODO: only output when necessary - len(params.clm_steps + params.mlm_steps) > 0
